...
import {
  X, MapPin, Navigation, Play, Share2, Bookmark,
  ChevronRight, Loader2, Film, ExternalLink
} from "lucide-react";
...
export function PlaceBottomSheet({
  ...
  // Fetch videos when place changes
  useEffect(() => {
    if (!place) {
      setVideos([]);
  ...
  const handleShare = useCallback(async () => {
    ...
    try {
      if (navigator.share) {
        await navigator.share({ title: place.name, text: `${place.name} — ${place.city || place.district || "Karnataka"}`, url });
      } else {
...
function VideoThumb({ video, onClick }: { video: IVideo; onClick: () => void }) {
  ...
  return (
    ...
    >
      ...
      {creator && (
        <p className="text-[10px] text-white/40 mt-0.5 truncate">{creator.fullName}</p>
      )}
    </button>
  );
}
