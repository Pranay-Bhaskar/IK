...
interface UsePlacesResult {
  ...
  loading: boolean;
  error: string | null;
  fetchByBounds: (bounds: { swLat: number; swLng: number; neLat: number; neLng: number }) => void;
  fetchNearby: (lat: number, lng: number, radiusKm?: number) => Promise<IPlace[]>;
  refetch: () => void;
}

export function usePlaces({ category, enabled = true }: UsePlacesOptions): UsePlacesResult {
