...
export default function MapPageClient() {
  ...
  // UPDATED: Logic to handle initial place zoom
  useEffect(() => {
    if (!isLoaded || !initialPlaceId || initialPlaceSet.current) return;

    const initializeMapPosition = async () => {
      // 1. Check if we already have the place in the current fetched list
      let place = safePlaces.find((p) => p._id === initialPlaceId);

      // 2. If not in current list, fetch it directly
      if (!place) {
        try {
          const res = await fetch(`/api/places/${initialPlaceId}`);
          const data = await res.json();
          if (data.success) place = data.data.place;
        } catch (e) {
          console.error("Failed to fetch initial place", e);
        }
      }
      ...
      if (place) {
        ...
        // Use a short timeout to ensure MapView is mounted
        setTimeout(() => {
          mapRef.current?.panTo(lat, lng, 16);
        }, 300);
      }
  ...
  const watchVideos = useCallback((video: IVideo) => {
    const placeId = typeof video.placeId === "object" ? video.placeId._id : video.placeId;
    router.push(`/place/${placeId}`);
  }, [router]);
  ...
  return (
    <div className="relative w-full h-[100dvh] bg-black overflow-hidden">
      {/* ── Search UI Overlay ── */}
      <div className="absolute top-0 left-0 right-0 z-20 px-4 pt-12 space-y-4">
        <div className="bg-black/40 backdrop-blur-md border border-white/10 rounded-2xl shadow-xl">
           <MapSearchBar places={safePlaces} onSelectPlace={handleSelectPlace} />
        </div>
        <MapFilterPanel activeCategory={category} onChange={setCategory} />
      </div>
