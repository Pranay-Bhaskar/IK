...
import {
  Bookmark, Share2, MapPin, Volume2, VolumeX,
  UserCircle2, Plus, Navigation,
  Footprints, Car, Bike, Bus, Sun
} from "lucide-react";
import { IVideo, TRAVEL_MODES } from "@/types";
import { formatCount, formatRelativeTime, cn } from "@/lib/utils";
import { CATEGORIES } from "@/constants";
...
export function VideoCard({ video, isActive, userLocation, scrollIndex = 0, totalCount = 1 }: Props) {
  ...
  // Extract the actual place ID securely to prevent 404 routing errors
  const pId = typeof video.placeId === "object" ? (video.placeId as any)?._id : video.placeId;
  ...
  // Check if video is already saved by the user
  useEffect(() => {
    if (!video._id) return;
    fetch(`/api/videos/save?videoId=${video._id}`)
      .then(r => r.json())
  ...
  const handleSave = useCallback(async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (data.success) {
        setSaved(data.data.saved);
        setSaveCount(c => data.data.saved ? c + 1 : Math.max(0, c - 1));
        toast.success(data.data.saved ? "Saved!" : "Removed");
      } else if (res.status === 401) {
  ...
  const handleShare = useCallback(async (e: React.MouseEvent) => {
    e.stopPropagation();
    if (!pId) {
      toast.error("Place link unavailable");
      return;
    }
    const url = `${window.location.origin}/place/${pId}`;
    try {
  ...
  const handleMapNavigation = useCallback((e: React.MouseEvent) => {
    e.stopPropagation();
    if (pId) {
      router.push(`/map?placeId=${pId}`);
    } else if (video.latitude && video.longitude) {
  ...
  }, [pId, video.latitude, video.longitude, router]);
      ...
      <div className="relative w-full h-dvh bg-black overflow-hidden select-none">
        ...
        <div className="absolute top-0 left-0 right-0 flex items-center justify-between px-4 pt-12 z-10 pointer-events-none">
          ...
          <div className="flex items-center gap-2 pointer-events-auto">
            <button
              ...
              className="w-9 h-9 rounded-full glass border border-white/10 flex items-center justify-center transition-all active:scale-95"
            >
              {muted ? <VolumeX className="w-4 h-4 text-white" /> : <Volume2 className="w-4 h-4 text-white" />}
            </button>
          </div>
        ...
        <div className="absolute right-3 bottom-36 flex flex-col items-center gap-4 z-10">
          ...
          <div className="flex flex-col items-center mb-1">
            <div className="w-12 h-12 rounded-2xl overflow-hidden border-2 border-white/70 bg-gradient-to-br from-purple-500 to-pink-500">
              {creator?.profileImage
                ? <img src={creator.profileImage} alt="Creator" className="w-full h-full object-cover" />
                : creator?.fullName
                  ? <div className="w-full h-full flex items-center justify-center text-white font-black text-lg">{creator.fullName[0]}</div>
                  : <UserCircle2 className="w-full h-full text-white p-1" />
              }
        ...
        {/* ── BOTTOM INFO ── */}
        <div className="absolute bottom-0 left-0 right-14 px-4 pb-24 z-10 pointer-events-auto">
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <button className="ml-auto border border-white/35 rounded-full px-3 py-1 text-[11px] font-bold text-white glass active:bg-white/10 transition-all">
              + Follow
            </button>
          </div>
          ...
          <button
            onClick={handleMapNavigation}
            className="flex items-center gap-1.5 glass border border-white/15 rounded-full px-3 py-1.5 mb-2 active:opacity-70 transition-opacity"
          >
            <MapPin className="w-3 h-3 text-[#a78bfa]" />
            <span className="text-xs text-white font-medium">{video.placeName}, {video.district}</span>
          </button>
          ...
          <div className="flex items-center gap-2 mb-3">
            {distKm !== null && (
              <button
                onClick={e => { e.stopPropagation(); setShowTravel(t=>!t); }}
                className="flex items-center gap-1.5 glass border border-white/10 rounded-full px-3 py-1.5 text-xs active:bg-white/10 transition-colors"
              >
                <Navigation className="w-3 h-3 text-[#a78bfa]" />
                <span className="text-white font-bold">{distKm} km away</span>
                {travelMins !== null && (
                  <>
                    ...
                    <span className="text-white/70 flex items-center gap-1">
                      {TRAVEL_ICONS[travelMode.mode] && (() => {
                        const Icon = TRAVEL_ICONS[travelMode.mode];
          ...
          {/* Travel mode picker */}
          {showTravel && distKm !== null && (
            <div className="glass border border-white/10 rounded-2xl p-2.5 grid grid-cols-4 gap-1.5 mb-3 animate-in slide-in-from-bottom-2 fade-in duration-200">
              {TRAVEL_MODES.map(mode => {
                const ModeIcon = TRAVEL_ICONS[mode.mode] || Car;
                const mins = Math.round((distKm / mode.speedKmh) * 60);
                const isOn = travelMode.mode === mode.mode;
          ...
          <div className="flex gap-2.5">
            <button
              onClick={e => { e.stopPropagation(); setShowItinerary(true); }}
              className="flex-1 bg-[#7c3aed] hover:bg-[#6d28d9] text-white text-xs font-black py-2.5 rounded-xl flex items-center justify-center gap-1.5 active:scale-[0.97] transition-all shadow-lg shadow-purple-900/30"
            >
              <Plus className="w-3.5 h-3.5" />
              Add to trip
            </button>
            <button
              onClick={handleMapNavigation}
              className="flex-1 glass border border-white/15 hover:bg-white/10 text-white text-xs font-bold py-2.5 rounded-xl flex items-center justify-center gap-1.5 active:scale-[0.97] transition-all"
            >
              <MapPin className="w-3.5 h-3.5" />
              View on Map
            </button>
...
function RailBtn({
  ...
  return (
    <button onClick={onClick} className="flex flex-col items-center gap-1 group">
      <div className={cn(
        "w-11 h-11 rounded-full border flex items-center justify-center transition-all group-active:scale-90",
        active ? "bg-[#7c3aed]/30 border-[#7c3aed]/50" : "glass border-white/10 group-hover:bg-white/10"
      )}>
        {children}
      </div>
      {count !== undefined && (
        <span className="text-white text-xs font-bold drop-shadow-md">{formatCount(count)}</span>
      )}
      <span className="text-white/55 text-[10px] font-medium drop-shadow-md">{label}</span>
    </button>
  );
}
