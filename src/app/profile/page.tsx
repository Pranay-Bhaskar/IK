    ...
    <div className="min-h-dvh bg-transparent pb-28 relative">
      ...
      <div className="relative z-10">
        ...
        <div className="relative px-4 pt-14 pb-0 overflow-hidden">
          ...
          {user.role === "EXPLORER" && (
            <div className="grid grid-cols-3 gap-2 mb-5">
              <StatCard value={2} label="Saved" color="text-white" />
              <StatCard value={1} label="Trips" color="text-white" />
...
export default function ProfilePage() {
  ...
  // New State to hold actual Explorer stats
  ...
  // Fetch Explorer Stats (Real actual counts from DB)
  useEffect(() => {
    if (user?.role === "EXPLORER") {
      const loadExplorerStats = async () => {
        try {
          // Fetching from your existing list endpoints to get the exact counts
          const [savedRes, itinRes] = await Promise.all([
            fetch("/api/videos/saved").then(r => r.json()).catch(() => ({})),
            fetch("/api/itineraries").then(r => r.json()).catch(() => ({}))
          ]);

          setExplorerStats({
            saved: Array.isArray(savedRes?.data?.saved) ? savedRes.data.saved.length : 0,
            trips: Array.isArray(itinRes?.data?.itineraries) ? itinRes.data.itineraries.length : 0,
            explored: 0 // Placeholder until you build a specific "explored places" tracker
          });
        } catch (err) {
  ...
  if (loading || !user) {
    return (
      <div className="h-dvh flex items-center justify-center bg-black relative">
        <div className="absolute inset-0 bg-gradient-to-b from-black/40 via-black/70 to-black/95 z-0 pointer-events-none" />
        <div className="relative z-10 w-20 h-20 rounded-3xl bg-white/5 border border-white/10 backdrop-blur-md flex items-center justify-center shadow-2xl">
  ...
  return (
    <div className="relative min-h-dvh bg-black pb-28">
      ...
      <div className="fixed inset-0 bg-gradient-to-b from-black/40 via-black/80 to-black z-0 pointer-events-none" />
      ...
      <div className="relative z-10">
        <div className="relative px-4 pt-14 pb-0 overflow-hidden">
          ...
          <div className="flex items-start justify-between mb-6 relative">
            <div className="flex items-center gap-4">
              <div className="relative">
                <div className="w-[72px] h-[72px] rounded-2xl bg-white/5 backdrop-blur-md flex items-center justify-center text-3xl font-black text-white border-2 border-white/10 shadow-2xl">
                  {user.fullName.charAt(0).toUpperCase()}
                </div>
                {user.role === "CREATOR" && (
                  <div className="absolute -bottom-2 -right-2 w-7 h-7 rounded-full bg-white border-2 border-black flex items-center justify-center shadow-lg">
                    <Camera className="w-3.5 h-3.5 text-black" />
                  </div>
                )}
              </div>
              <div>
                <h1 className="text-xl font-black tracking-tight text-white leading-tight drop-shadow-md">{user.fullName}</h1>
                <p className="text-[11px] font-bold text-zinc-400 mt-1 drop-shadow-md uppercase tracking-wider">
                  @{user.fullName.replace(/\s+/g, "").toLowerCase()}
                </p>
                <div className="flex items-center gap-2 mt-2 flex-wrap">
                  <RoleBadge role={user.role} />
                  {user.district && (
                    <span className="flex items-center gap-1 text-[10px] text-zinc-300 font-bold bg-white/5 backdrop-blur-md border border-white/10 px-2.5 py-1 rounded-full shadow-sm uppercase tracking-wider">
                      <MapPin className="w-2.5 h-2.5" />{user.district}
                    </span>
                  )}
                </div>
            ...
            <div className="flex flex-col gap-2">
              <button
                onClick={logout}
                className="w-10 h-10 rounded-xl bg-white/5 backdrop-blur-md border border-white/10 flex items-center justify-center transition-colors hover:bg-white/10 shadow-lg active:scale-95"
                title="Sign out"
              >
                <LogOut className="w-4 h-4 text-white" />
              </button>
          ...
          {/* ── Explorer Stats (Now Linked to State) ── */}
          {user.role === "EXPLORER" && (
            <div className="grid grid-cols-3 gap-3 mb-6">
              <StatCard value={explorerStats.saved} label="Saved" color="text-white" />
              <StatCard value={explorerStats.trips} label="Trips" color="text-white" />
              <StatCard value={explorerStats.explored} label="Explored" color="text-white" />
            </div>
          )}
        </div>
        ...
        {user.role === "CREATOR" && (
          <div className="px-4 mt-2">
            ...
            <div className="flex bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-1.5 gap-1.5 mb-5 shadow-xl">
              {(["all", "APPROVED", "PENDING", "REJECTED"] as VideoTab[]).map(tab => (
                <button
                  key={tab}
                  onClick={() => handleTabChange(tab)}
                ...
                >
                  {TAB_LABELS[tab]}
                  <span className="ml-1 opacity-60 font-bold">({counts[tab]})</span>
                </button>
              ))}
            </div>
            ...
            {videosLoading ? (
              <div className="space-y-3">
                {[1, 2, 3].map(i => (
                  <div key={i} className="h-28 skeleton rounded-2xl bg-white/5" />
                ))}
              </div>
            ) : videos.length === 0 ? (
              <EmptyVideos tab={activeTab} onUpload={() => router.push("/creator/upload")} />
            ) : (
              <div className="space-y-3">
                {videos.map(video => (
                  <VideoListCard
            ...
            <button
              ...
              className="w-full mt-5 flex items-center justify-center gap-2 border-2 border-dashed border-white/20 rounded-2xl py-4 text-sm font-black text-white bg-white/5 hover:bg-white/10 backdrop-blur-md active:scale-[0.98] transition-all shadow-xl"
            >
              <Camera className="w-4 h-4" />
              Upload new story
            </button>
        ...
        {/* ── Explorer Menu Section ── */}
        {user.role === "EXPLORER" && (
          <div className="px-4 mt-2 space-y-6">
            <MenuSection title="MY CONTENT">
              <MenuItem icon={Bookmark} label="Saved places"   sub="Bookmarked locations"      onClick={() => router.push("/saved")} />
              <MenuItem icon={Route}    label="My trips"       sub="Planned itineraries"       onClick={() => router.push("/itineraries")} />
              <MenuItem icon={Video}    label="Saved videos"   sub="Videos you bookmarked"     onClick={() => router.push("/saved")} />
            </MenuSection>
...
function RoleBadge({ role }: { role: string }) {
  const cfg: Record<string, { icon: React.ElementType; label: string; cls: string }> = {
    EXPLORER: { icon: User,   label: "Explorer", cls: "bg-white/10 text-white border-white/20" },
    CREATOR:  { icon: Camera, label: "Creator",  cls: "bg-white text-black border-white" },
    ADMIN:    { icon: Shield, label: "Admin",    cls: "bg-white/10 text-white border-white/20" },
  };
  const c = cfg[role];
  if (!c) return null;
  return (
    <span className={cn("flex items-center gap-1.5 text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-full border shadow-sm backdrop-blur-md", c.cls)}>
...
function StatCard({ value, label, color }: { value: number; label: string; color: string }) {
  return (
    <div className="rounded-2xl p-4 text-center border bg-white/5 backdrop-blur-md border-white/10 shadow-xl">
      <div className={cn("text-2xl font-black tracking-tight drop-shadow-sm", color)}>{value}</div>
      <div className="text-[9px] text-zinc-400 mt-1 font-black tracking-[0.2em] uppercase">{label}</div>
    </div>
  );
}
...
function MenuSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <p className="text-[10px] font-black text-zinc-500 tracking-[0.2em] uppercase mb-3 ml-1 drop-shadow-md">{title}</p>
...
function MenuItem({
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "w-full bg-white/5 backdrop-blur-md rounded-2xl border px-4 py-4 flex items-center gap-4 active:scale-[0.98] transition-all text-left shadow-lg",
        danger ? "border-rose-500/20 hover:bg-rose-500/10" : "border-white/10 hover:border-white/20 hover:bg-white/10"
      )}
    >
      <div className={cn(
        "w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 border",
        danger ? "bg-rose-500/10 border-rose-500/20" : "bg-black/40 border-white/5"
      )}>
        <Icon
          style={{ width: 18, height: 18 }}
          className={danger ? "text-rose-400" : "text-white"}
        />
      </div>
      <div className="flex-1 min-w-0">
        <p className={cn("text-sm font-black tracking-tight", danger ? "text-rose-400" : "text-white")}>{label}</p>
        <p className="text-[11px] font-bold text-zinc-400 mt-0.5">{sub}</p>
      </div>
      {!danger && <ChevronRight className="w-4 h-4 text-zinc-500" />}
    </button>
  );
...
function VideoListCard({ video, onClick }: { video: IVideo; onClick: () => void }) {
  ...
  const statusCls = 
    video.status === "APPROVED" ? "bg-white text-black border-white" :
    video.status === "PENDING" ? "bg-white/10 text-white border-white/20" :
  ...
  return (
    <button
      onClick={onClick}
      className="w-full bg-white/5 backdrop-blur-md border border-white/10 shadow-lg rounded-2xl overflow-hidden flex text-left active:scale-[0.98] transition-all hover:border-white/20 hover:bg-white/10"
    >
      <div className="w-[80px] h-[96px] bg-black/40 flex-shrink-0 relative overflow-hidden border-r border-white/5">
        {video.thumbnailUrl ? (
          <img src={video.thumbnailUrl} alt="" className="w-full h-full object-cover grayscale-[15%]" />
        ) : (
          <div className="w-full h-full flex items-center justify-center text-2xl grayscale">
            {cat?.emoji || "🎬"}
          </div>
        )}
      </div>
      ...
      <div className="flex-1 p-3.5 min-w-0 flex flex-col justify-between">
        <div>
          <p className="text-sm font-black text-white truncate tracking-tight drop-shadow-sm">{video.title}</p>
        ...
        <div className="flex items-center justify-between mt-2">
          <div className={cn(
            "flex items-center gap-1 text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full border backdrop-blur-md",
            statusCls
          )}>
            <StatusIcon style={{ width: 10, height: 10 }} />
            {statusLabel}
        ...
        {video.status === "REJECTED" && video.rejectionReason && (
          <p className="text-[10px] text-rose-400 mt-1 line-clamp-1 italic font-bold drop-shadow-sm">
            "{video.rejectionReason}"
          </p>
        )}
      </div>
...
function EmptyVideos({ tab, onUpload }: { tab: VideoTab; onUpload: () => void }) {
  ...
  const m = msgs[tab];
  return (
    <div className="py-16 flex flex-col items-center text-center bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl shadow-xl">
      <div className="text-5xl mb-4 grayscale drop-shadow-lg">{m.emoji}</div>
      <p className="text-sm font-black tracking-tight text-white mb-1 drop-shadow-md">{m.title}</p>
      <p className="text-xs font-bold text-zinc-400 mb-6 drop-shadow-md max-w-[200px] leading-relaxed">{m.sub}</p>
      {tab === "all" && (
        <button
          onClick={onUpload}
          className="flex items-center gap-2 bg-white text-black font-black px-6 py-3 rounded-2xl text-sm transition-transform active:scale-[0.98] shadow-xl"
        >
          <Camera className="w-4 h-4" />
          Upload now
        </button>
