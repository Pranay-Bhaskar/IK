...
import { useState, useRef, useCallback } from "react";
import { useRouter } from "next/navigation";
import { Upload, X, CheckCircle2, CloudUpload, MapPin, Tag, ChevronDown, ArrowLeft, Loader2, Info } from "lucide-react";
import { uploadVideo, generateVideoPath } from "@/lib/cloudinary/upload";
import { useAuth } from "@/features/auth/AuthContext";
import { CATEGORIES, KARNATAKA_DISTRICTS, ALLOWED_VIDEO_TYPES, MAX_VIDEO_SIZE } from "@/constants";
import { cn } from "@/lib/utils";
...
// Updated to Monochromatic Glassmorphism
const inputCls = "w-full bg-black/40 backdrop-blur-md border border-white/10 rounded-xl px-4 py-3 text-sm font-bold text-white placeholder-zinc-500 focus:border-white focus:bg-white/5 transition-all outline-none";
...
export default function UploadPage() {
    ...
    return (
      <div className="relative min-h-dvh flex flex-col items-center justify-center px-8 text-center">
        ...
        <div className="relative z-10 flex flex-col items-center w-full max-w-sm">
          ...
          <h2 className="text-2xl font-black text-white mb-2 drop-shadow-md">Uploaded!</h2>
          <p className="text-sm font-bold text-zinc-300 mb-1">Your story is under review.</p>
          <p className="text-xs font-medium text-zinc-500 mb-8">It appears in the feed once approved by our team.</p>
          <div className="flex flex-col gap-3 w-full">
            <button onClick={() => router.push("/creator/content")} className="w-full bg-white text-black font-black py-3.5 rounded-2xl active:scale-[0.98] transition-all shadow-xl">
              View my videos
            </button>
            <button onClick={reset} className="w-full bg-white/5 border border-white/10 backdrop-blur-md text-white font-bold py-3.5 rounded-2xl hover:bg-white/10 active:scale-[0.98] transition-all">
              Upload another
            </button>
          </div>
        </div>
    ...
    return (
      <div className="relative min-h-dvh flex flex-col items-center justify-center px-8 text-center">
        ...
        <div className="relative z-10 w-full max-w-sm flex flex-col items-center">
          ...
          <h2 className="text-xl font-black text-white mb-1 drop-shadow-sm">Uploading</h2>
          <p className="text-4xl font-black text-white my-4 drop-shadow-lg">{progress}%</p>
          <div className="w-full bg-white/10 border border-white/10 rounded-full h-3 mb-4 overflow-hidden backdrop-blur-md shadow-inner">
            <div
              className="bg-white h-full rounded-full transition-all duration-300 ease-out"
              style={{ width: `${progress}%` }}
            />
          </div>
          <p className="text-xs font-bold uppercase tracking-wider text-zinc-500">Don&apos;t close this tab</p>
        </div>
      </div>
    );
  }
  ...
  if (step === "pick") {
    return (
      <div className="relative min-h-dvh flex flex-col">
        ...
        <div className="relative z-10 flex items-center gap-3 px-4 pt-14 pb-5">
          <button onClick={() => router.back()} className="w-9 h-9 rounded-full bg-white/5 border border-white/10 backdrop-blur-md flex items-center justify-center hover:bg-white/10 transition-colors">
            <ArrowLeft className="w-4 h-4 text-white" />
          </button>
          <h1 className="text-xl font-black text-white drop-shadow-md">Upload story</h1>
        </div>
        
        <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-6 pb-28">
          {error && <div className="w-full mb-4 bg-rose-500/10 border border-rose-500/20 backdrop-blur-md rounded-xl px-4 py-3 text-sm font-bold text-rose-400 shadow-lg">{error}</div>}
          ...
          <div
            ...
            onClick={() => fileInputRef.current?.click()}
            className={cn(
              "w-full rounded-3xl border-2 border-dashed flex flex-col items-center justify-center gap-5 transition-all cursor-pointer p-10 backdrop-blur-md shadow-2xl",
              dragOver ? "border-white bg-white/10 scale-[1.02]" : "border-white/20 bg-black/40 hover:border-white/40 hover:bg-white/5"
            )}
            style={{ minHeight: "55dvh" }}
          >
            <div className="w-20 h-20 rounded-3xl bg-white/5 border border-white/10 shadow-inner flex items-center justify-center">
              <CloudUpload className="w-10 h-10 text-white" />
            </div>
            <div className="text-center">
              <p className="text-white font-black text-base mb-1 drop-shadow-sm">Tap to upload video</p>
              <p className="text-sm font-medium text-zinc-400">MP4, MOV, WEBM · max 200MB</p>
              <p className="text-[11px] font-bold uppercase tracking-wider text-zinc-500 mt-1">Vertical 9:16 works best</p>
            </div>
            <div className="bg-white rounded-2xl px-6 py-3 shadow-xl active:scale-95 transition-transform">
              <span className="text-sm font-black text-black">Choose file</span>
            </div>
          </div>
          <input ref={fileInputRef} type="file" accept="video/mp4,video/quicktime,video/webm" className="hidden" onChange={e => { const f = e.target.files?.[0]; if (f) handleFile(f); }} />
        </div>
  ...
  // Details form
  return (
    <div className="relative min-h-dvh overflow-y-auto pb-32">
      <div className="fixed inset-0 bg-gradient-to-b from-black/40 via-black/70 to-black/95 z-0 pointer-events-none" />

      {/* Sticky Header */}
      <div className="relative z-20 flex items-center gap-3 px-4 pt-14 pb-4 border-b border-white/10 bg-black/60 backdrop-blur-xl sticky top-0 shadow-sm">
        <button onClick={() => { setStep("pick"); setFile(null); setPreview(null); }} className="w-9 h-9 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-white/10 transition-colors">
          <ArrowLeft className="w-4 h-4 text-white" />
        </button>
        <h1 className="text-xl font-black text-white flex-1 drop-shadow-md">Story details</h1>
      </div>

      {/* File preview */}
      <div className="relative z-10 flex items-center gap-4 px-4 py-4 border-b border-white/10 bg-black/20 backdrop-blur-md">
        {preview && (
          <div className="w-14 h-20 rounded-xl border border-white/10 overflow-hidden flex-shrink-0 relative shadow-lg">
            <video src={preview} className="w-full h-full object-cover grayscale-[20%]" muted />
            <button onClick={() => { setStep("pick"); setFile(null); setPreview(null); }} className="absolute top-1 right-1 w-5 h-5 rounded-full bg-black/60 backdrop-blur-md border border-white/20 flex items-center justify-center hover:bg-black/80 transition-colors">
              <X className="w-3 h-3 text-white" />
            </button>
          </div>
        )}
        <div>
          <p className="text-sm font-bold text-white truncate max-w-[220px] drop-shadow-sm">{file?.name}</p>
          <p className="text-[11px] font-bold uppercase tracking-wider text-zinc-500 mt-1">{file ? `${(file.size / 1024 / 1024).toFixed(1)} MB` : ""}</p>
        </div>
      </div>

      <div className="relative z-10">
        {error && <div className="mx-4 mt-4 bg-rose-500/10 border border-rose-500/20 backdrop-blur-md rounded-xl px-4 py-3 text-sm font-bold text-rose-400 shadow-lg">{error}</div>}
        ...
        <form onSubmit={handleSubmit} className="px-4 py-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Place name *">
              <div className="relative">
                ...
                <input value={form.placeName} onChange={e => setForm({...form, placeName: e.target.value})} placeholder="Jog Falls" className={inputCls + " pl-10"} />
              </div>
            </Field>
            <Field label="District *">
              <div className="relative">
          ...
          <Field label="Tags (comma separated)">
            <div className="relative">
              ...
              <input value={form.tags} onChange={e => setForm({...form, tags: e.target.value})} placeholder="monsoon, hidden gem, waterfall" className={inputCls + " pl-10"} />
            </div>
          </Field>

          {/* Info Banner - Monochromatic styling */}
          <div className="bg-white/5 border border-white/10 backdrop-blur-md rounded-2xl p-4 flex gap-3 shadow-lg mt-2">
            <Info className="w-4 h-4 text-white flex-shrink-0 mt-0.5" />
            <p className="text-xs font-medium text-zinc-300 leading-relaxed">Your video will be reviewed before appearing in the feed. Usually 24–48 hours.</p>
          </div>

          <button type="submit" className="w-full bg-white text-black font-black py-4 rounded-2xl transition-all shadow-xl active:scale-[0.98] flex items-center justify-center gap-2 mt-4">
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="text-[10px] font-black uppercase tracking-wider text-zinc-400 mb-2 block">{label}</label>
      {children}
    </div>
  );
}